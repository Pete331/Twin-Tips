...
test("deleting an account", async (t) => {
  ...
  await db.Fixture.create([
    {
      ...
      ascore: 80,
      winner: "Adelaide",
      date: new Date("2096-03-05T09:00:00Z"),
    },
    {
      id: 960002,
      year: YEAR,
      ...
      ascore: 80,
      winner: "Geelong",
      date: new Date("2096-03-06T09:00:00Z"),
    },
  ]);
  ...
  await sessions().insertMany([
    sessionFor("cat-phone", U.cat),
    sessionFor("cat-laptop", U.cat),
    sessionFor("bob-phone", U.bob),
  ]);
  ...
  const siteWinnings = async (user) =>
    (await db.Tip.findOne({ user: user._id, season: YEAR, round: 1 }).lean())
      ?.winnings;
  const poolWinnings = async (user) =>
    (
      await db.LeagueRoundResult.findOne({
        league: pool._id,
        season: YEAR,
        round: 1,
        user: user._id,
  ...
  await t.test(
    "somebody running a league with others in it is asked to hand it over",
    async () => {
      ...
      assert.equal(
        destroyed,
        before,
        "and they are still signed in to do the handing over"
      );
      assert.equal(
        await db.LeagueMembership.countDocuments({ user: U.ann._id }),
        1
      );
    }
  );
  ...
  await t.test(
    "a player who was paid deletes, and the round stays as it was paid",
    async () => {
      assert.equal(
        await siteWinnings(U.bob),
        2,
        "precondition: bob and cat split the site pool"
      );
      ...
      assert.equal(
        await siteWinnings(U.bob),
        2,
        "bob's site-pool share is what he was paid"
      );
      ...
      assert.ok(
        await db.Tip.exists({ user: U.cat._id }),
        "cat's tip is still there"
      );
    }
  );
  ...
  await t.test(
    "they are out of every league, and their results stay",
    async () => {
      assert.equal(
        await db.LeagueMembership.countDocuments({ user: U.cat._id }),
        0
      );
      assert.equal(
        await db.LeagueRoundResult.countDocuments({ user: U.cat._id }),
        1
      );
    }
  );
  ...
  await t.test(
    "every device they were signed in on is signed out",
    async () => {
      const left = await sessions().find({}).toArray();
      assert.deepEqual(left.map((s) => s._id).sort(), ["bob-phone"]);
      assert.equal(
        await sessionUsers.findSessionUser(U.cat._id),
        null,
  ...
  await t.test("their old address can sign up again", async () => {
    await db.User.create({
      username: "cat_returns",
      email: "cat@delete.test",
      password: "x",
      firstName: "Cat",
      lastName: "Again",
      favTeam: 1,
  ...
  await t.test(
    "the site's tables show a former player only where they played",
    async () => {
      const played = await globalLadder.roundDetail(YEAR, 1);
      const former = played.standings.filter((r) =>
        /^Former player /.test(r.username)
      );
      ...
      assert.equal(
        after.standings.some((r) => /^Former player /.test(r.username)),
        false,
  ...
  await t.test(
    "and a stored ladder taken before the deletion agrees",
    async () => {
      const fay = await db.User.create({
        username: "del_fay",
        email: "fay@delete.test",
        password: "x",
        firstName: "fay",
        lastName: "Deleter",
        favTeam: 1,
      ...
      assert.equal(
        cached.rebuilt,
        false,
        "precondition: read from the stored ladder"
      );
  ...
  await t.test(
    "somebody running a league on their own can delete, and it closes",
    async () => {
      ...
      assert.equal(
        stillOpen.deletedAt,
        null,
        "the pool dan was only a member of is untouched"
      );
    }
  );
});
