...
module.exports = {
    ...
    if (req.isAuthenticated()) {
      ...
      req.logout((err) => {
        if (err) {
          return res
            .status(500)
            .json({ success: false, message: "Unable to log out" });
        }
        res
          .status(200)
  ...
  register: (req, res) => {
    ...
    if (!validPassword(password)) {
      return res.status(400).json({
        success: false,
        message:
          "Password requires a minimum of eight characters, at least one letter and one number",
      });
    }
    ...
    if (isReservedUsername(username)) {
      return res
        .status(400)
        .json({ success: false, message: "That username is not available." });
    }
    ...
    db.User.findOne({ email: email })
      .then(async (user) => {
        ...
        const takenUsername = await db.User.findOne({ username }).collation(
          USERNAME_COLLATION
        );
  ...
  changeUsername: async (req, res) => {
    ...
    try {
      ...
      const taken = await db.User.findOne({ username }).collation(
        USERNAME_COLLATION
      );
      ...
      res
        .status(200)
        .json({ success: true, message: "Username updated.", username });
    } catch (err) {
  ...
  changePassword: async (req, res) => {
    ...
    if (currentPassword === newPassword) {
      return res
        .status(400)
        .json({ success: false, message: "That is already your password." });
    }
  ...
  forgotPassword: async (req, res) => {
    ...
    const sameAnswer = () =>
      res.status(200).json({
        success: true,
        message:
          "If that email is registered, a reset link is on its way. It will expire in 30 min!",
      });
    ...
    try {
      ...
      res.status(503).json({
        success: false,
        message:
          "We couldn't send the reset email just now. Please try again shortly, or ask an admin.",
      });
    }
  },
    ...
    try {
      if (!token) {
        return res.status(422).json({
          success: false,
          message: "Password reset link is either invalid or expired!",
        });
      }
